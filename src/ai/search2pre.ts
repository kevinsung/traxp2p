 ...
 *
 * A byte-for-byte copy of src/ai/search2.ts as of commit ad67e68 (2026-07-30):
 * the build shipping before the evaluation-pricing round. Registered as `pre` in
 * all three CLIs, so a hand-ported eval term can be gated **paired** against
 * exactly what it replaced — both arms over identical game seeds in one process,
 * seeing the same analyst stream and the same machine load.
 *
 * It imports the live `./fastboard`, deliberately: the board is not what is
 * changing, and a copy of the search alone already gets its own transposition
 * table, eval cache and stamp lane, which is what the pairing needs. It also
 * imports the live `WEIGHTS`, so this snapshot is only frozen as long as no
 * *existing* weight's value changes — adding new ones is safe.
 *
 * `base` is NOT a valid pairing arm for this round: it is the 14da60e build and
 * is four promoted rounds stale.
 *
 * Dead code as far as the app bundle is concerned: src/ai/worker.ts imports only
 * ./search2, so nothing here is shipped.
 */
